...
static uint8_t PollSwitch(SwitchCtx_t *sw, uint8_t currentlyPressed)
{
    ...
    {
        if (!sw->falling && ((g_msTick - (sw->lockoutEnd)) < 0x80000000U))
        {
            sw->falling = 1U;
...
uint8_t HMS_Poll_Fan_Switch(volatile uint8_t *o_fan_status, uint8_t hardware_update)
{
    uint8_t fanPressed = (!(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0400U))->IDR & (0x1U << (0U))));


    if (PollSwitch(&s_fanSw, fanPressed))
    {
        s_fanEnabled = (uint8_t)(!s_fanEnabled);
    *o_fan_status = s_fanEnabled;
...
void HMS_Poll_Light_Switch(volatile uint8_t *o_light_status)
{
    uint8_t lightPressed = (!(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U))->IDR & (0x1U << (10U))));

    if (PollSwitch(&s_lightSw, lightPressed))
    {

        if (s_uartLightOffValid && ((g_msTick - s_uartLightOffTime) <= 10000U))
        {
            s_lightEnabled = 0U;
            *o_light_status = 0U;
        ...
        {
            if ((!(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U))->IDR & (0x1U << (8U)))))
            {
                s_lightEnabled = 0U;
                *o_light_status = 0U;
                (((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0000U))->ODR |= (0x1U << (9U)));
